import { fileParamsSchema } from '../schemas/file.js';
import { uploadFile, getFileUrl, deleteFile } from '../services/fileService.js';
import prisma from '../config/database.js';
const filesRoutes = async (fastify) => {
    // Upload file
    fastify.post('/files/upload', {
        ...
        const applicationId = data.fields.applicationId?.value;
        if (!applicationId) {
            return reply.code(400).send({ error: 'Application ID is required' });
        }
        try {
            const file = await uploadFile(buffer, data.filename, data.mimetype, applicationId);

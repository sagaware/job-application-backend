...
import { moveFileToApplication, getFileTypeFromS3Key } from '../services/fileService.js'

const SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/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'
...
async function sendSlackNotification(application: any, portfolioUrl?: string) {
  try {
    ...
    const name = personalInfo.fullName || application.name || 'Unknown Applicant'
    const type = typeMap[internshipDetails.type] || internshipDetails.type || 'Unknown'
    const period = periodMap[internshipDetails.period] || internshipDetails.period || 'Unknown'
...
const applicationsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/applications', {
    ...
    if (applicationWithFiles) {
      ...
      const portfolioFile = applicationWithFiles.files.find(
        file => {
          const fileType = getFileTypeFromS3Key(file.s3Key)
          return fileType === 'portfolio' && file.mimeType === 'application/pdf'
        }
      )
      ...
      if (portfolioFile) {
        ...
        const baseUrl = process.env.BASE_URL || 'https://lk0k8gs4gksk84gsk4s0k8ok.saga.dk'
        portfolioUrl = `${baseUrl}/api/files/${portfolioFile.id}/view`
      }

      // Send notification (don't await - let it run in background)
      sendSlackNotification(applicationWithFiles, portfolioUrl).catch(error => {
